...
**Acceptance Criteria**:
- [ ] 커플 플랜에 따라 오늘 읽을 성경 구절 자동 계산
- [ ] AI 질문 생성 (자정, 하이브리드 전략 - 비용 최적화)
- [ ] 푸시 알림: "오늘의 질문이 도착했어요! 📖" 전송
- [ ] 홈 화면에서 오늘의 말씀 카드 확인
- [ ] 성경 구절: 커플이 설정한 분량 (1-10절 또는 1-5장)
- [ ] AI 생성 질문: 1개 (성경 읽기 유도 + 소감 작성 가이드)

#### US-003: 소감 작성 (용어 변경)
**As a** 사용자
**I want to** 말씀을 읽고 질문에 대한 소감을 작성하고 싶다
**So that** 3-5분 안에 부담 없이 완료할 수 있다

**Acceptance Criteria**:
- [ ] 성경 구절 읽기 (폰트 크기 조절 가능)
- [ ] AI 질문 확인 (이미 생성되어 있음)
- [ ] 텍스트 입력 (자유 형식, 최대 500자)
...
- **AI API**: Gemini 1.5 Flash (하이브리드 전략)
- **실행 방식**: 매일 자정 Cron Job (권장)
...
3. **분량 체크 (하이브리드 전략)**:
   - **5절 이하**: 전체 텍스트 → Gemini 호출
   - **6-50절**: 핵심 구절 3-5개 추출 → Gemini 호출
   - **51절 이상**: 템플릿 질문 사용 (Gemini 안 씀)
4. 로컬 JSON 파일에서 해당 구절 조회
5. Gemini API 호출 (해당되는 경우)
   - 입력: 전체 텍스트 또는 핵심 구절
   - 프롬프트: 커플 상태별 맞춤 질문 생성
6. 생성된 질문 검증 (신학적 적절성 필터)
7. 커플별 `daily_verses` 테이블에 저장
8. `couples.daily_verse_plan`의 `current_book`, `current_chapter`, `current_verse` 업데이트

**실행 타이밍**:
- **매일 자정 Cron Job** (권장) ← 유지!
- 이유:
  - 질문 먼저 도착 → 성경 읽기 동기 부여
  - "오늘의 질문이 도착했어요!" 푸시 알림
  - 차별화 포인트 (AI 맞춤 질문)
